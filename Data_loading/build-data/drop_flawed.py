...
def main():
    image_base,label_base='data/top14-sup-transformed/allset/images/','data/top14-sup-transformed/allset/labels/'
    x,y=os.listdir(image_base),os.listdir(label_base)
    #open('D:\WWF\data-check-list\check-list-part2\check-all-part2.txt','r')
    drop_list_x=np.unique(np.array([line2.replace('\n','',1)+'.jpg' for line2 in open('D:\WWF\data-check-list\check-list-part2\check-all-part2.txt')])).tolist()
    drop_list_y=np.unique(np.array([line2.replace('\n','',1)+'.txt' for line2 in open('D:\WWF\data-check-list\check-list-part2\check-all-part2.txt')])).tolist()

  
    droped_x=[i for i in x if i not in drop_list_x]
    
    droped_y=[i for i in y if i not in drop_list_y]
    for i in zip(droped_x,droped_y):
        assert(os.path.splitext(i[0])[0]==os.path.splitext(i[1])[0])

    x_train,x_tar,y_train,y_tar=train_test_split(droped_x,droped_y,test_size=0.2,random_state=2021)
    train_folder_img,train_folder_txt='data/top14-sup-transformed/conservation/train/images/','data/top14-sup-transformed/conservation/train/labels/'
    test_folder_img,test_folder_txt='data/top14-sup-transformed/conservation/val/images/','data/top14-sup-transformed/conservation/val/labels/'
